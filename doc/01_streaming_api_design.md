...
services like live transcription in mind. I recommend you read previous
...
inward and see what decisions we have to make in terms of our API. Our
system is a bidirectional streaming system - meaning that data is streamed
from client to server and server to client. This prevents us from doing
...
transcribe service does. But that feels like a spicier option that's
...
say any more about that. If there are any other options let me know, this feels fairly
comprehensive.

For this we'll start by looking at the first two, their pros and
cons and then the act of designing the API.

## Websockets

Websockets work pretty much everywhere which is a plus in their
favour. You establish a HTTP connection and upgrade it to a raw TCP
socket. This means you get all the pros and cons of raw TCP sockets.

1. Easy to use
2. Completely untyped

There's also an extra negative, if you expect a browser based frontend
...
validate a user's allowed to interact with your service. You'll either send
it in the first message or enforce wss only and use a URL parameter.

## gRPC 

gRPC works over HTTP/2 meaning you can only use it where you have HTTP/2, which
while it may be in a lot of places it's not everywhere. I've previously had
...
work. Then you end up offering a websocket API or moving things from client to
the backend.

However, it does offer some form of typing for your API even if it is Protobuf
(I won't cover the issues with Protobuf they've been discussed to death).
Additionally, there's a gRPC ecosystem you can interact with which can give
some benefits of its own.

## Which to Choose?

So you don't need to choose just one! You can choose to do both and
multiplex your requests. Later on, we'll do this, but for now, we'll choose to
do websockets. This keeps things simpler and makes our API usable in more
contexts.

## Designing the API

Now it's time to think about what we want to do and see what design falls
out as a result. We already know we're sending audio in but how are we sending
it? 

Audio files?

Audio files are simple for the client which is a plus. However, it's
complicated on our end and not every file is streamable. If you've tried to stream
...
service! Also, things like microphones don't tend to output audio in a file meaning a
user would need a streaming encoder for that codec. So what's our next choice?

PCM Data!

Pulse Code Modulation (PCM) data is audio data in its simplest form. A list of
samples (typically little endian), where channels are interleaved. We don't need
to know anything except the number of channels, the bits per sample and the
...
With this in mind let's add a starting message to our API. We can also use this
...
After this is sent I'll assume we'll send the audio data itself. This
will be done via raw bytes because any audio streaming APIs that take
base64 data in json are an affront to all that is holy. You have a TCP socket
make use of it!

When audio is coming in we're choosing segments from it to run
...
and if we haven't detected the end of the segment they need a way to signal
us to stop. Then we can force an end of the segment and send the final results back.
...
The client can also instantly disconnect if they don't care about the
...
With the inputs initially decided, let's work out the outputs!
...
the user. We'll do a "simple" API in this project to cover that use-case.

But, often segments may be different sizes and then it's useful to know the start
and end times of the segment - we'll refer to this as the "segmented API". 
...
We could just use segment for both the simple and the segmented
...
Looking at the segmented API, we may want our service to report
...
return output for every X seconds added to the segment. Because of
...
results, but this is a potentially dangerous API decision. Partial inferences
take time and doing them too frequently could cause your real-time system to
become not real-time as it falls further and further behind. In this case you
...
With this in mind our `SegmentOutput` turns to:
...
For the purpose of debugging or fancier interactions, you may want to expose
events where some processing has started. A semi-common example I've seen in
Speech-To-Text systems is to send an event when speech start and speech end is
...
These can also be quite useful for debugging. Did we get no output because the
...
streaming audio APIs. In future installments, we'll be writing up
